...
from actions.booking.booking import get_user_request_buses, get_selected_bus, book_ticket, get_booking
import pandas as pd
...
def confirm_booking():
    if request.method == 'POST':
        ...
        record = {'user_id': user_id, 'bus_number': bus_number, 'window_left': window_left,
                  'window_right': window_right, 'left': left, 'right': right, 'booked_date': book_date}
        booking = book_ticket(record)
        session.pop('book_date', None)
        bus_details = get_selected_bus(bus_number)
        return render_template('booking_done.html', bus=bus_details, price=price, booking=booking)
